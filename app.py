...
if uploaded_file:
    ...
    # Mapping colonne dal tuo file: t, FS, DV, FR
    # Utilizziamo FS (Phase Stability) per l'analisi di stabilità
    ...
    # Identifichiamo il primo punto di superamento della soglia 0.55 nel dataset
    try:
        critical_idx = next(i for i, v in enumerate(l_path) if v >= 0.55)
        # Il Lead Time è la differenza tra la fine del segnale e il momento della rilevazione
        dynamic_lead = time_axis[-1] - time_axis[critical_idx]
        lead_label = f"{dynamic_lead:.2f} units" # Es. minuti o secondi in base al file
    except StopIteration:
        dynamic_lead = 0
        lead_label = "N/A (Stable)"

    # --- KPI METRICS ---
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Current L (ϕ)", f"{current_phi:.4f}")
    m2.metric("Target (ϕ*)", "0.5500")
    m3.metric("Specific Lead", lead_label) # Risultato predittivo specifico per questo CSV
    m4.metric("Status", "TRANSITION" if current_phi >= 0.55 else "NOMINAL")

    st.divider()

    # --- VISUAL ANALYTICS ---
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=time_axis[window:], y=l_path, line=dict(color='#000000', width=1.2)))
    fig.add_hline(y=0.55, line_dash="dot", line_color="#ff3b30", opacity=0.4)
    
    if dynamic_lead > 0:
        # Evidenzia il punto di predizione specifica
        fig.add_vline(x=time_axis[critical_idx + window], line_color="#007bff", opacity=0.3)
    ...
    st.markdown("<p style='text-align:center; color:#ccc; margin-top:50px;'>LOAD CSV DATASET FOR SPECIFIC PREDICTIVE LEAD ANALYSIS</p>", unsafe_allow_html=True)
